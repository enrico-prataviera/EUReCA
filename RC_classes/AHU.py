...
class AirHandlingUnit:
    ...
    def AHUCalc(self,t,G_da_vent,AHUOnOff,AHUHUM,Sens_Recovery_eff,Lat_Recovery_eff,OutAirRatio,T_ext,x_ext,T_int,x_int,T_sup,x_sup):
        ...
        if SatCond == False:
            wrn(f'WARNING  AHUCalc method, bd {self.bd_name}, time step {t}, Zone conditions outside saturation limit')
        SatCond, psat = checkSatCond(self.T_supAHU[t],self.x_sup,self.p_atm)
        if SatCond == False:
            wrn(f'WARNING  AHUCalc method, bd {self.bd_name}, time step {t}, Supply conditions outside saturation limit')
